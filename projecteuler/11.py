...
def find_off(off, x, y):
    if off == Off.t_l:
        if x - 3 < 0 or y - 3 < 0:
            return 0
        else:
            return [lst[x][y], lst[x - 1][y - 1], lst[x - 2][y - 2], lst[x - 3][y - 3]]
    elif off == Off.t_t:
        if y - 3 < 0:
            return 0
        else:
            return [lst[x][y], lst[x][y - 1], lst[x][y - 2], lst[x][y - 3]]
    elif off == Off.t_r:
        if x + 3 >= row_len or y - 3 < 0:
            return 0
        else:
            return [lst[x][y], lst[x + 1][y - 1], lst[x + 2][y - 2], lst[x + 3][y - 3]]
    elif off == Off.r:
        if x + 3 >= row_len:
            ...
            return [lst[x][y], lst[x + 1][y], lst[x + 2][y], lst[x + 3][y]]
    elif off == Off.b_r:
        if x + 3 >= row_len or y + 3 >= col_len:
            return 0
        else:
            return [lst[x][y], lst[x + 1][y + 1], lst[x + 2][y + 2], lst[x + 3][y + 3]]
    elif off == Off.b_b:
...
def largest_multiple(x, y):
    largest = 0
    for i in range(0, 8):
        prim_list = find_off(i, x, y)
        if prim_list != 0:
            total = prim_list[0] * prim_list[1] * prim_list[2] * prim_list[3]
